...
interface ListPageProps {
    ...
    isAllData?: boolean //是否需要加载本身的ticket
    onAItemClick?: (assetInfo: AssetInfo) => void
}

const pagesize = 4;

const ListPage: React.FC<ListPageProps> = ({ style, isAllData, onAItemClick }) => {
    ...
    useEffect(
        () => {
            ...
            AssetList(OnAssetList, 1, pagesize, currentToken ? currentToken.token : undefined)
        }, []
    )
        ...
        if (resp) {
            ...
            for (let i = 0; i < resp.data.length; i++) {
                if (!list.find(asset => asset.assetId === resp.data[i].assetId)) {
                    list.push(getStdAssetInfo(resp.data[i]))
                }
            }
            setCurrentList(list)
    ...
    const handScroll = async () => {
        ...
        if (!loading && hasMore) {
            ...
            setLoading(true)
            const currentToken = GetToken();
            AssetList(OnAssetList, 1 + currentPageIndex, pagesize, (currentToken && (!isAllData)) ? currentToken.token : undefined)
        }
    }
    const uniqueItems = currentList.filter((item) => {
        const seenIds = new Set<number>();
        return !seenIds.has(item.assetId) && seenIds.add(item.assetId);
    });
            ...
            {uniqueItems.map((item, index) => (
                <GridItem key={index} onClick={() => onItemClick(item)}>
                    <div style={ItemStyle}>
                        ...
                        <div style={{ width: '90%', height: 'auto' }}><p style={{ margin: '10px 0 15px 0', ...ListTitletyle }}>{item.assetName}</p></div>
                    </div>
                </GridItem>
            ))}
        </Grid>
