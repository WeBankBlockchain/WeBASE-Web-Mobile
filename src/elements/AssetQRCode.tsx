...
const AssetQRCode: React.FC<AssetQRCodeProps> = ({ style, code }) => {
    ...
    const [QRCodeCanvasStyle, setQRCodeCanvasStyle] = useState<React.CSSProperties>({ marginTop: '5%', position: 'absolute', width: '50%', height: 'auto', border: '10px solid #ffffff' })

    const onGetFoldWindow = (isFlod: boolean) => {
        if (isFlod) {
            setQRCodeCanvasStyle({ marginTop: '3%', position: 'absolute', width: '35%', height: 'auto', border: '10px solid #ffffff' })
        }
    }
